...
import { attendeeLogin, attendeeRegister, attendeeForgotPassword, attendeeResetPassword } from "../lib/api";
  ...
  const handleCustomLogin = async (e) => {
    ...
    try {
      ...
      if (mode === "register") {
        ...
        toast.success(`Account created! Welcome, ${attendee.name}`);
      } else {
      ...
      <DialogContent className="max-w-sm rounded-2xl p-6">
        <DialogHeader>
          <DialogTitle className="font-display text-2xl font-extrabold tracking-tight text-center">
            {mode === "login" ? "Welcome back" : mode === "forgot-password" ? "Reset password" : mode === "reset-password" ? "Set New Password" : "Create an account"}
          </DialogTitle>
        </DialogHeader>
